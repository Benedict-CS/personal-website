...
| `ACCESS_BLOCK_IP_PREFIXES` | Comma-separated IPv4 prefixes for blocked clients. **When non-empty:** by default blocked IPs get **403 on every path** (including `/`, `/blog`, and APIs), so they cannot generate analytics page views. Use a trailing dot, e.g. `140.113.194.` for that /24 only. Empty = no blocking. **Selective (legacy):** set `ACCESS_BLOCK_ADMIN_ONLY=1` so public pages and the small set of public GET APIs still work for blocked IPs. **Force selective:** `ACCESS_BLOCK_PUBLIC=0`. Restart after changing. **Docker:** pass into the container (`docker-compose` `environment`), not only on the host. | Not set |
